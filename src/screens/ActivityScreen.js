...
const initialActivities = [{
  ...
  images: ['https://images.unsplash.com/photo-1552664730-d307ca884978?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1531482615713-2afd69097998?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1557804506-669a67965ba0?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1556761175-b413da4baf72?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=800&h=600&fit=crop', 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=800&h=600&fit=crop'],
  participants: 12580,
...
export default function ActivityScreen({
      ...
      if (a.id === id) {
        if (a.joined) {
          showAppAlert(t('common.ok'), t('screens.activity.actions.quitConfirm'), [{
            ...
            onPress: () => {
              setActivities(prev => prev.map(item => item.id === id ? {
                ...item,
                joined: false,
  ...
  const tabs = [t('screens.activity.tabs.all'), t('screens.activity.tabs.hot'), t('screens.activity.tabs.new'), t('screens.activity.tabs.ended'), t('screens.activity.tabs.mine')];
  return <SafeAreaView style={styles.container} edges={['top']}>
      {/* 头部 */}
      <View style={styles.header}>
        {isFromProfile ? <TouchableOpacity onPress={() => navigation.navigate('Main', {
        screen: '我的'
      })} style={styles.backBtn} hitSlop={{
        top: 15,
        bottom: 15,
        left: 15,
        right: 15
        ...
        <TouchableOpacity onPress={() => navigation.navigate('CreateActivity')} style={styles.createBtn} hitSlop={{
        top: 15,
        bottom: 15,
        left: 15,
        right: 15
      ...
      {/* 标签栏 - 仅在非从我的页面进入时显示 */}
      {!isFromProfile && <View style={styles.tabBar}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabScroll}>
            {tabs.map(tab => <TouchableOpacity key={tab} style={[styles.tabItem, activeTab === tab && styles.tabItemActive]} onPress={() => setActiveTab(tab)}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {filteredActivities.length > 0 ? filteredActivities.map(item => <View key={item.id} style={styles.activityCard}>
              {/* 封面图区域 - 可点击查看图片 */}
              <TouchableOpacity style={styles.coverImageContainer} onPress={() => handleImagePress(item)} activeOpacity={0.9}>
                <Image source={{
            uri: item.image
          }} style={styles.activityImage} />
                
                {/* 图片数量角标 - 仅多图时显示 */}
                {Boolean(item.images && item.images.length > 1) && <View style={styles.imageCountBadge}>
              ...
              <View style={styles.activityInfo}>
                ...
                {Boolean(item.type === 'offline' && item.address) ? <View style={styles.addressRow}>
                ...
                {Boolean(item.joined && item.progress) ? <View style={styles.progressRow}>
                    <Text style={styles.progressLabel}>{t('screens.activity.progress')}</Text>
                    <Text style={styles.progressText}>{item.progress}</Text>
                  </View> : null}
                <TouchableOpacity style={[styles.joinBtn, item.joined && styles.joinedBtn, item.status === 'ended' && styles.endedBtn]} onPress={() => item.status !== 'ended' && handleJoinActivity(item.id)} disabled={item.status === 'ended'}>
      ...
      <Modal visible={showImageViewer} animationType="fade" transparent={false} statusBarTranslucent>
        <View style={styles.imageViewerContainer}>
          ...
          setCurrentImageIndex(index);
            ...
            {viewerImages.map((image, index) => <View key={index} style={styles.imageSlide}>
                <Image source={{
              uri: image
          ...
          <View style={styles.imageViewerFooter}>
            {viewerImages.map((_, index) => <View key={index} style={[styles.imageIndicator, index === currentImageIndex && styles.imageIndicatorActive]} />)}
          </View>
        </View>
      </Modal>
    </SafeAreaView>;
