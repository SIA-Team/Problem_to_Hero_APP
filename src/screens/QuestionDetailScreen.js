...
export default function QuestionDetailScreen({ navigation, route }) {
  ...
  const answerTabs = React.useMemo(() => {
    ...
    return [
      ...
      `${t('screens.questionDetail.tabs.comments')} (${commentsTotal || questionData?.commentCount || 0})`,
      t('screens.questionDetail.tabs.invite')
    ];
  ...
  const currentReplyComment =
    commentsList.find(comment => String(comment.id) === String(currentCommentId)) ||
  ...
  const normalizeQuestionDetail = (question) => {
    ...
    const normalizedAuthorAvatar =
      ...
      question.avatar ??
      null;
    const normalizedImageUrls = Array.isArray(question.imageUrls)
      ? question.imageUrls
      : (Array.isArray(question.images) ? question.images : []);
    const normalizedTopicNames = Array.isArray(question.topicNames)
      ? question.topicNames.filter(Boolean)
      : (
  ...
  const normalizeAnswerItem = (answer) => {
    ...
    const normalizedLiked =
      ...
      answer.isLiked ??
      false;
    const normalizedDisliked =
      answer.disliked ??
      answer.isDisliked ??
      false;
    const normalizedAdopted =
      answer.adopted ??
      (answer.isAccepted === 1 || answer.isAccepted === true);
    const normalizedCollectCount =
      answer.collectCount ??
      answer.bookmarkCount ??
      answer.bookmark_count ??
      answer.bookmarks ??
      0;
    const normalizedSuperLikeCount =
      answer.superLikeCount ??
      answer.superLikes ??
      0;
  ...
  const syncAnswerInteractionStates = (answers = []) => {
    ...
    setAnswerBookmarked(prev => ({ ...prev, ...collectedState }));
  };
  ...
  const normalizeCommentItem = (comment) => {
    ...
    const normalizedShareCount =
      ...
      0;
    const normalizedCollectCount =
      comment.collectCount ??
      comment.bookmarkCount ??
      comment.bookmarks ??
      0;
    const normalizedSuperLikeCount =
    ...
    const normalizedTime =
      comment.time ??
      formatTime(comment.createTime || comment.createdAt);
  ...
  const syncCommentInteractionStates = (comments = []) => {
    ...
    setCommentLiked(prev => ({ ...prev, ...likedState }));
    setCommentCollected(prev => ({ ...prev, ...collectedState }));
    setCommentDisliked(prev => ({ ...prev, ...dislikedState }));
  };
  ...
  const updateCommentInCaches = (commentId, updater) => {
    setCommentsCache(prevCache => {
      ...
      Object.keys(nextCache).forEach(sortKey => {
        ...
        nextCache[sortKey] = {
          ...currentEntry,
          list: currentEntry.list.map(comment => (
  ...
  const buildCommentFromMutationResponse = (currentComment, responseData, fallbackValues = {}) => {
    ...
    return normalizeCommentItem({
      ...
      collectCount: payload?.collectCount ?? payload?.bookmarkCount ?? payload?.bookmarks ?? fallbackValues.collectCount ?? currentComment.collectCount,
  ...
  const syncSupplementInteractionStates = (supplements = []) => {
    ...
    supplements.forEach(supplement => {
      ...
      collectedState[supplement.id] = !!(
        supplement.collected ??
        supplement.isCollected ??
        supplement.bookmarked ??
        supplement.isBookmarked
      );
    });

    setSuppLiked(prev => ({ ...prev, ...likedState }));
    setSuppDisliked(prev => ({ ...prev, ...dislikedState }));
    setSuppBookmarked(prev => ({ ...prev, ...collectedState }));
  };
  ...
  const getSupplementById = (supplementId) => {
    ...
    return (
      supplementsCache[supplementsSortBy]?.list?.find(item => String(item?.id) === String(supplementId)) ||
      Object.values(supplementsCache)
        .flatMap(entry => entry?.list || [])
        .find(item => String(item?.id) === String(supplementId)) ||
      null
    );
  };
  ...
  const buildSupplementMutationResult = (currentSupplement = {}, responseData, fallbackValues = {}) => {
    ...
    const liked =
      typeof responseData === 'boolean'
        ? responseData
        : (payload.liked ?? payload.isLiked ?? fallbackValues.liked ?? currentSupplement.liked ?? false);
    const disliked =
      typeof responseData === 'boolean' && fallbackValues.disliked !== undefined
        ? responseData
        : (payload.disliked ?? payload.isDisliked ?? fallbackValues.disliked ?? currentSupplement.disliked ?? false);
    const collected =
      payload.collected ??
      payload.isCollected ??
      payload.bookmarked ??
      payload.isBookmarked ??
      fallbackValues.collected ??
      currentSupplement.collected ??
      false;
    const likeCount =
      Number(payload.likeCount ?? payload.likes ?? fallbackValues.likeCount ?? currentSupplement.likeCount ?? currentSupplement.likes) || 0;
    const dislikeCount =
      Number(payload.dislikeCount ?? payload.dislikes ?? fallbackValues.dislikeCount ?? currentSupplement.dislikeCount ?? currentSupplement.dislikes) || 0;
    const collectCount =
      Number(
        payload.collectCount ??
        payload.bookmarkCount ??
        payload.bookmarks ??
        fallbackValues.collectCount ??
        currentSupplement.collectCount ??
        currentSupplement.bookmarkCount ??
        currentSupplement.bookmarks
      ) || 0;
  ...
  const buildQuestionMutationResult = (currentQuestionDetail = {}, responseData, fallbackValues = {}) => {
    ...
    const liked =
      typeof responseData === 'boolean' && fallbackValues.liked !== undefined
        ? responseData
        : (payload.liked ?? payload.isLiked ?? fallbackValues.liked ?? currentQuestionDetail.liked ?? false);
    const collected =
      typeof responseData === 'boolean' && fallbackValues.collected !== undefined
        ? responseData
        : (
            payload.collected ??
            payload.isCollected ??
            ...
            payload.isBookmarked ??
            fallbackValues.collected ??
            currentQuestionDetail.collected ??
            false
          );
    const disliked =
      typeof responseData === 'boolean' && fallbackValues.disliked !== undefined
        ...
        : (
            payload.disliked ??
            payload.isDisliked ??
            fallbackValues.disliked ??
            currentQuestionDetail.disliked ??
            false
          );
    const likeCount =
      Number(payload.likeCount ?? payload.likes ?? fallbackValues.likeCount ?? currentQuestionDetail.likeCount ?? currentQuestionDetail.likes) || 0;
    const collectCount =
      Number(
        payload.collectCount ??
        payload.bookmarkCount ??
        payload.bookmarks ??
        fallbackValues.collectCount ??
        currentQuestionDetail.collectCount ??
        currentQuestionDetail.bookmarkCount ??
        currentQuestionDetail.bookmarks
      ) || 0;
    const dislikeCount =
      Number(payload.dislikeCount ?? payload.dislikes ?? fallbackValues.dislikeCount ?? currentQuestionDetail.dislikeCount ?? currentQuestionDetail.dislikes) || 0;
  ...
  const handleCommentDislike = async (commentId) => {
    ...
    const currentState = commentDisliked[commentId] !== undefined
      ? commentDisliked[commentId]
      : !!comment?.disliked;
    ...
    try {
      ...
      if (response && response.code === 200) {
        ...
        updateCommentInCaches(commentId, currentComment => {
          const currentDislikeCount = Number(currentComment.dislikeCount ?? currentComment.dislikes ?? 0) || 0;
  ...
  const handleCommentLike = async (commentId) => {
    ...
    const currentState = commentLiked[commentId] !== undefined
    ...
    try {
      ...
      if (response && response.code === 200) {
        ...
        updateCommentInCaches(commentId, currentComment => {
          const currentLikeCount = Number(currentComment.likeCount ?? currentComment.likes ?? 0) || 0;
  ...
  const handleCommentCollect = async (commentId) => {
    ...
    const currentState = commentCollected[commentId] !== undefined
      ? commentCollected[commentId]
      : !!comment?.collected;
    const newState = !currentState;
    ...
    try {
      ...
      const response = newState
        ? await commentApi.collectComment(commentId)
        : await commentApi.uncollectComment(commentId);

      if (response && response.code === 200) {
        showToast(newState ? '已收藏' : '已取消收藏', 'success');

        updateCommentInCaches(commentId, currentComment => {
          const currentCollectCount = Number(currentComment.collectCount ?? currentComment.bookmarkCount ?? currentComment.bookmarks ?? 0) || 0;
  ...
  const handleAnswerCollect = async (answerId) => {
    ...
    // 立即更新UI状态，提供即时反馈
    const currentState = answerBookmarked[answerId] !== undefined
      ? answerBookmarked[answerId]
      : !!answer?.collected;
    const newState = !currentState;
  ...
  const handleAnswerLike = async (answerId) => {
    ...
    // 立即更新UI状态，提供即时反馈
    const currentState = answerLiked[answerId] !== undefined
      ? answerLiked[answerId]
      : !!answer?.liked;
    const newState = !currentState;
  ...
  const handleAnswerDislike = async (answerId) => {
    ...
    // 立即更新UI状态，提供即时反馈
    const currentState = answerDisliked[answerId] !== undefined
      ? answerDisliked[answerId]
      : !!answer?.disliked;
    const newState = !currentState;
  ...
  const findAnswerByAnyId = (targetId) => {
    ...
    return answersList.find(item => {
      const candidateIds = [item?.id, item?.answerId, item?.answer_id]
  ...
  const handleAnswerAdopt = async (questionId, answerId) => {
    ...
    if (
      ...
      String(answerQuestionId) !== String(routeQuestionId)
    ) {
    ...
    // 获取当前采纳状态
    const currentState = answerAdopted[answerStateKey] !== undefined
      ? answerAdopted[answerStateKey]
  ...
  const handleSupplementDislike = async (supplementId) => {
    try {
      ...
      const currentDisliked = suppDisliked[supplementId] ?? currentSupplement?.disliked ?? false;
      const baseDislikeCount = Number(currentSupplement?.dislikeCount ?? currentSupplement?.dislikes) || 0;
      const nextDisliked = !currentDisliked;
      const fallbackDislikeCount = Math.max(baseDislikeCount + (nextDisliked ? 1 : -1), 0);
      const response = currentDisliked
    ...
    try {
      ...
      const currentSupplement =
        ...
        Object.values(supplementsCache)
          .flatMap(entry => entry?.list || [])
          .find(item => String(item?.id) === String(supplementId));
      const currentLiked = suppLiked[supplementId] ?? currentSupplement?.liked ?? false;
      const baseLikeCount = Number(currentSupplement?.likeCount ?? currentSupplement?.likes) || 0;
  ...
  const handleSupplementCollect = async (supplementId) => {
    try {
      ...
      const response = currentCollected
  ...
  const handleQuestionLike = async () => {
    ...
    try {
      const currentLiked = liked.main ?? questionData.liked ?? false;
      const baseLikeCount = Number(questionData.likeCount ?? questionData.likes) || 0;
      const nextLiked = !currentLiked;
      const fallbackLikeCount = Math.max(baseLikeCount + (nextLiked ? 1 : -1), 0);
      const response = currentLiked
        ? await questionApi.unlikeQuestion(questionData.id)
        : await questionApi.likeQuestion(questionData.id);
    ...
    try {
      ...
      const baseCollectCount =
        Number(
          ...
          questionData.bookmarkCount ??
          questionData.bookmarks
        ) || 0;
      const nextCollected = !currentCollected;
      const fallbackCollectCount = Math.max(baseCollectCount + (nextCollected ? 1 : -1), 0);
      const response = currentCollected
        ? await questionApi.uncollectQuestion(questionData.id)
        : await questionApi.collectQuestion(questionData.id);
  ...
  const handleQuestionDislike = async () => {
    ...
    try {
      const currentDisliked = liked.dislike ?? questionData.disliked ?? false;
      const baseDislikeCount = Number(questionData.dislikeCount ?? questionData.dislikes) || 0;
      const nextDisliked = !currentDisliked;
      const fallbackDislikeCount = Math.max(baseDislikeCount + (nextDisliked ? 1 : -1), 0);
      const response = currentDisliked
        ? await questionApi.undislikeQuestion(questionData.id)
        : await questionApi.dislikeQuestion(questionData.id);
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          <View style={[styles.sortFilterBar, { display: activeTab && activeTab !== '邀请' ? 'flex' : 'none' }]}>
            <View style={styles.sortFilterLeft}>
              {activeTab && activeTab.includes('补充') ? (
                ...
                <>
                  <TouchableOpacity 
                    style={[styles.sortFilterBtn, commentsSortBy === 'likes' && styles.sortFilterBtnActive]}
                    onPress={() => handleCommentsSortChange('likes')}
                  >
                  ...
                  <TouchableOpacity 
                    style={[styles.sortFilterBtn, commentsSortBy === 'newest' && styles.sortFilterBtnActive]}
                    onPress={() => handleCommentsSortChange('newest')}
                  >
          ...
          {activeTab && activeTab.includes('补充') ? (
                ...
                >
                  ...
                  <View style={styles.suppFooter}>
                    <View style={styles.suppFooterLeft}>
                      ...
                      >
                        <Ionicons
                          name={(suppLiked[item.id] ?? item.liked) ? "thumbs-up" : "thumbs-up-outline"}
                          size={16}
                          color={(suppLiked[item.id] ?? item.liked) ? "#ef4444" : "#6b7280"}
                        />
                        <Text style={[styles.suppActionText, (suppLiked[item.id] ?? item.liked) && { color: '#ef4444' }]}>
                          {getInteractionDisplayCount(item.likeCount ?? item.likes, item.liked, suppLiked[item.id])}
                        </Text>
                      </TouchableOpacity>
                      <TouchableOpacity 
                        style={styles.suppActionBtn}
                      ...
                      >
                        <Ionicons
                          name={(suppBookmarked[item.id] ?? item.collected) ? "star" : "star-outline"}
                          size={16}
                          color={(suppBookmarked[item.id] ?? item.collected) ? "#f59e0b" : "#6b7280"}
                        />
                        <Text style={[styles.suppActionText, (suppBookmarked[item.id] ?? item.collected) && { color: '#f59e0b' }]}>
            ...
            <>
              ...
              {commentsList.map(comment => {
                ...
                const commentLikeCount = getInteractionDisplayCount(
                  comment.likeCount || comment.likes || 0,
                  comment.liked,
                  commentLiked[comment.id]
                );
                const commentCollectCount = getInteractionDisplayCount(
                  comment.collectCount || comment.bookmarkCount || comment.bookmarks || 0,
                  comment.collected,
                  commentCollected[comment.id]
                );
                const commentDislikeCount = getInteractionDisplayCount(
                  comment.dislikeCount || comment.dislikes || 0,
                  comment.disliked,
                  commentDisliked[comment.id]
                );
                    ...
                    <View style={styles.commentFooter}>
                      <View style={styles.commentFooterLeft}>
                        ...
                        >
                          ...
                          <Text style={[styles.commentActionText, isCommentLiked && { color: '#ef4444' }]}>{commentLikeCount}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                          style={styles.commentActionBtn}
                        ...
                        <TouchableOpacity
                          style={styles.commentActionBtn}
                          onPress={() => handleCommentCollect(comment.id)}
                          disabled={commentCollectLoading[comment.id]}
                        >
                          <Ionicons name={isCommentCollected ? "star" : "star-outline"} size={14} color={isCommentCollected ? "#f59e0b" : "#9ca3af"} />
                          <Text style={[styles.commentActionText, isCommentCollected && { color: '#f59e0b' }]}>{commentCollectCount}</Text>
                        </TouchableOpacity>
                      </View>
                      <View style={styles.commentFooterRight}>
                        <TouchableOpacity
                          style={styles.commentActionBtn}
                          onPress={() => handleCommentDislike(comment.id)}
                          disabled={commentDislikeLoading[comment.id]}
                        >
                          <Ionicons name={isCommentDisliked ? "thumbs-down" : "thumbs-down-outline"} size={14} color={isCommentDisliked ? "#6b7280" : "#9ca3af"} />
                          <Text style={[styles.commentActionText, isCommentDisliked && { color: '#6b7280' }]}>{commentDislikeCount}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                          style={styles.commentActionBtn}
            ...
            const likeCount = getInteractionDisplayCount(
              answer.likeCount || answer.like_count || answer.likes || 0,
              answer.liked,
              answerLiked[answer.id]
            );
            const collectCount = getInteractionDisplayCount(
              answer.collectCount || answer.bookmarkCount || answer.bookmark_count || answer.bookmarks || 0,
              answer.collected,
              answerBookmarked[answer.id]
            );
            const dislikeCount = getInteractionDisplayCount(
              answer.dislikeCount || answer.dislike_count || answer.dislikes || 0,
              answer.disliked,
              answerDisliked[answer.id]
            );

            return (
            <TouchableOpacity key={answerStateKey} style={[styles.answerCard, isAdopted && styles.answerCardAdopted]} onPress={() => navigation.navigate('AnswerDetail', { answer, defaultTab: 'supplements' })}>
                  ...
                  <View style={styles.answerAuthorRow}>
                    ...
                    <TouchableOpacity 
                      style={[
                        ...
                        isAdopted && styles.adoptAnswerBtnActive,
                        isAdoptLoading && styles.adoptAnswerBtnLoading
                      ]}
                      onPress={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      <Text style={[
                        ...
                        isAdopted && styles.adoptAnswerBtnTextActive
                      ]}>
              ...
              <View style={styles.answerFooter}>
                <View style={styles.answerFooterLeft}>
                  ...
                  >
                    ...
                    <Text style={[styles.answerActionText, isLiked && { color: '#ef4444' }]}>
                      {likeCount}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
                  ...
                  >
                    ...
                    />
                    <Text style={[styles.answerActionText, isCollected && { color: '#f59e0b' }]}>
                      {collectCount}
                    </Text>
                ...
                <View style={styles.answerFooterRight}>
                  ...
                  >
                    <Ionicons name={isDisliked ? "thumbs-down" : "thumbs-down-outline"} size={16} color="#6b7280" />
                    <Text style={[styles.answerActionText, isDisliked && { color: '#6b7280' }]}>
                      {dislikeCount}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity style={styles.bottomActionBtn} onPress={handleQuestionLike}>
          <Ionicons
            name={(liked.main ?? questionData?.liked) ? "thumbs-up" : "thumbs-up-outline"}
            size={20}
            color={(liked.main ?? questionData?.liked) ? "#ef4444" : "#6b7280"}
          />
          <Text style={[styles.bottomActionText, (liked.main ?? questionData?.liked) && { color: '#ef4444' }]}>
        ...
        <TouchableOpacity style={styles.bottomActionBtn} onPress={handleQuestionCollect}>
          <Ionicons
            name={(bookmarked ?? questionData?.collected) ? "star" : "star-outline"}
            size={20}
            color={(bookmarked ?? questionData?.collected) ? "#f59e0b" : "#6b7280"}
          />
          <Text style={[styles.bottomActionText, (bookmarked ?? questionData?.collected) && { color: '#f59e0b' }]}>
            {getInteractionDisplayCount(
              questionData?.collectCount ?? questionData?.bookmarkCount ?? questionData?.bookmarks,
              questionData?.collected,
              bookmarked
            )}
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.bottomActionBtn} onPress={handleQuestionDislike}>
          <Ionicons
            name={(liked.dislike ?? questionData?.disliked) ? "thumbs-down" : "thumbs-down-outline"}
            size={20}
            color={(liked.dislike ?? questionData?.disliked) ? "#6b7280" : "#9ca3af"}
          />
          <Text style={[styles.bottomActionText, (liked.dislike ?? questionData?.disliked) && { color: '#6b7280' }]}>
        ...
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowSuppMoreModal(false)}>
          <View style={[styles.suppMoreModal, { paddingBottom: insets.bottom + 30 }]}>
            ...
            <View style={styles.suppMoreActionList}>
              ...
              >
                <Ionicons 
                  name={(suppDisliked[currentSuppId] ?? getSupplementById(currentSuppId)?.disliked) ? "thumbs-down" : "thumbs-down-outline"} 
                  size={22} 
                  color={(suppDisliked[currentSuppId] ?? getSupplementById(currentSuppId)?.disliked) ? "#ef4444" : "#6b7280"} 
                />
                <Text style={[
                  styles.suppMoreActionText,
                  (suppDisliked[currentSuppId] ?? getSupplementById(currentSuppId)?.disliked) && { color: '#ef4444' }
                ]}>
                  {(suppDisliked[currentSuppId] ?? getSupplementById(currentSuppId)?.disliked) ? '取消踩' : '踩一下'}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={styles.suppMoreActionItem}
            ...
            {currentReplyComment && (
              <View style={styles.originalCommentCard}>
                <View style={styles.originalCommentHeader}>
                  <Avatar 
                    uri={currentReplyComment.userAvatar || currentReplyComment.avatar}
                    name={currentReplyComment.userName || currentReplyComment.userNickname || currentReplyComment.author}
                    size={32}
                  />
