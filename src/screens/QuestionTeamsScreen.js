...
export default function QuestionTeamsScreen({
  route,
  navigation
}) {
  const {
  ...
  const handleApplyJoin = (teamId, teamName) => {
    showAppAlert(t('screens.questionTeams.alerts.applyTitle'), t('screens.questionTeams.alerts.applyMessage').replace('{name}', teamName), [{
  ...
  return <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {teams.map(team => <View key={team.id} style={styles.teamCard}>
            <TouchableOpacity style={styles.teamCardHeader} onPress={() => handleTeamPress(team)} activeOpacity={0.7} accessible={true} accessibilityRole="button">
              <Avatar uri={team.avatar} name={team.name} size={52} />
              <View style={styles.teamInfo}>
                <View style={styles.teamTitleRow}>
                  <Text style={styles.teamName} numberOfLines={1}>{team.name}</Text>
                  {Boolean(team.isJoined) && <View style={styles.joinedBadge}>
                      <Ionicons name="checkmark-circle" size={12} color="#22c55e" />
                      <Text style={styles.joinedText}>{t('screens.questionTeams.badges.joined')}</Text>
                    </View>}
                  {Boolean(team.isPending) && <View style={styles.pendingBadge}>
                ...
                {/* 显示用户角色 */}
                {Boolean(team.isJoined) && <View style={styles.userRoleRow}>
            ...
            <View style={styles.teamFooter}>
              ...
              backgroundColor: team.activeColor + '15'
                ...
                color: team.activeColor
              }]}>{t(`screens.questionTeams.activeLevel.${team.activeLevel}`)}</Text>
                </View>
              </View>
              
              {!team.isJoined && !team.isPending && <TouchableOpacity style={styles.applyBtn} onPress={() => handleApplyJoin(team.id, team.name)} activeOpacity={0.6} accessible={true} accessibilityRole="button" hitSlop={{
            top: 10,
            bottom: 10,
            left: 10,
            right: 10
              ...
              {Boolean(team.isPending) && <View style={styles.pendingTag}>
              ...
              {Boolean(team.isJoined) && <TouchableOpacity style={styles.enterTag} onPress={() => handleTeamPress(team)} accessible={true} accessibilityRole="button">
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              <View style={styles.inputGroup}>
                ...
                <TextInput style={styles.input} placeholder={t('screens.questionTeams.createModal.namePlaceholder')} value={newTeamName} onChangeText={setNewTeamName} maxLength={20} />
              </View>
              <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>{t('screens.questionTeams.createModal.descLabel')}</Text>
                <TextInput style={[styles.input, styles.textArea]} placeholder={t('screens.questionTeams.createModal.descPlaceholder')} value={newTeamDesc} onChangeText={setNewTeamDesc} multiline numberOfLines={3} maxLength={100} />
              </View>
