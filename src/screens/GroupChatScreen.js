...
const initialMessages = [{
  ...
  content: '我觉得3个月入门完全可以，关键是要坚持每天练习',
  time: '25',
  likes: 28,
  dislikes: 2,
  shares: 8,
  ...
  content: '推荐先从基础语法开始，然后学pandas和numpy，这两个库在数据分析中用得最多',
  time: '60',
  likes: 45,
  dislikes: 3,
  shares: 12,
  ...
  id: 4,
  author: '转行成功',
  avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=msg4',
  content: '我就是文科转行的，现在已经做了2年数据分析了，加油！',
  time: '120',
  likes: 67,
  dislikes: 1,
  shares: 18,
  ...
  id: 5,
  author: '编程导师',
  avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=msg5',
  content: '建议找一个实际项目来练手，比如爬虫或者数据可视化，这样学得更快',
  time: '180',
  likes: 89,
  dislikes: 2,
  shares: 25,
  ...
  return <SafeAreaView style={styles.container}>
        ...
        <View style={styles.messagesSection}>
          ...
          <View style={styles.sortFilterBar}>
            <View style={styles.sortFilterLeft}>
              <TouchableOpacity style={[styles.sortFilterBtn, activeTab === 'featured' && styles.sortFilterBtnActive]} onPress={() => setActiveTab('featured')}>
              ...
              <TouchableOpacity style={[styles.sortFilterBtn, activeTab === 'all' && styles.sortFilterBtnActive]} onPress={() => setActiveTab('all')}>
          ...
          {filteredMessages.map(msg => <View key={msg.id} style={styles.messageCard}>
              ...
              <View style={styles.msgActions}>
                <TouchableOpacity style={styles.msgActionBtn} onPress={() => setLiked({
              ...liked,
              [msg.id]: !liked[msg.id]
            })}>
                  <Ionicons name={liked[msg.id] ? "thumbs-up" : "thumbs-up-outline"} size={14} color={liked[msg.id] ? "#ef4444" : "#6b7280"} />
                  <Text style={[styles.msgActionText, liked[msg.id] && {
                color: '#ef4444'
              }]}>{msg.likes + (liked[msg.id] ? 1 : 0)}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.msgActionBtn} onPress={() => setDisliked({
              ...disliked,
              [msg.id]: !disliked[msg.id]
                  ...
                  <Text style={[styles.msgActionText, disliked[msg.id] && {
                color: '#3b82f6'
              }]}>{msg.dislikes + (disliked[msg.id] ? 1 : 0)}</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity style={styles.msgActionBtn} onPress={() => setBookmarked({
              ...bookmarked,
              [msg.id]: !bookmarked[msg.id]
                  ...
                  <Text style={[styles.msgActionText, bookmarked[msg.id] && {
                color: '#f59e0b'
              }]}>{msg.bookmarks + (bookmarked[msg.id] ? 1 : 0)}</Text>
                </TouchableOpacity>
      ...
      <Modal visible={showReplyModal} transparent animationType="slide">
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowReplyModal(false)}>
          <TouchableOpacity activeOpacity={1} style={styles.replyModal}>
            ...
            {Boolean(replyTarget) && <View style={styles.replyTargetCard}>
                <Image source={{
              uri: replyTarget.avatar
            ...
            <View style={styles.replyInputWrapper}>
              <TextInput style={styles.replyInput} placeholder={t('screens.groupChat.replyPlaceholder').replace('{author}', replyTarget?.author || '')} value={replyText} onChangeText={setReplyText} multiline autoFocus />
            </View>
      ...
      <View style={styles.bottomBar}>
        <View style={styles.inputWrapper}>
          <TextInput style={styles.input} placeholder={t('screens.groupChat.inputPlaceholder')} value={inputText} onChangeText={setInputText} multiline />
        </View>
