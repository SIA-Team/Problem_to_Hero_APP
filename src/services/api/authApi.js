...
const authApi = {
  ...
  tokenLogin: async () => {
    ...
    try {
      ...
      if (response.code === 200 && response.data) {
        console.log('\n📦 data 字段内容:');
        console.log('   新 Token:', response.data.token ? `${response.data.token.substring(0, 30)}...` : '❌ 无');
        console.log('   Token 有效期 (expiresIn):', response.data.expiresIn, '小时');
