 ...
 */
export default function SupplementAnswerModal({ visible, onClose, answer, questionId, onSuccess }) {
    ...
    try {
      ...
      if (response?.data?.code === 200 && response?.data?.data) {
        ...
        setImages(prev => 
          prev.map(img => 
            img.uri === imageUri 
              ? { uri: imageUri, url: response.data.data, uploading: false }
              : img
          )
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 如果有图片，添加图片URL数组
      if (images.length > 0) {
        requestData.imageUrls = images.map(img => img.url).filter(Boolean);
      }

      // 如果后端修复了bug，需要传递 questionId，可以取消下面的注释
      // if (questionId) {
      ...
      console.log('  questionId:', questionId, '(暂不传递，等待后端修复)');
      ...
      // 如果是 question_id 相关的错误，给出特殊提示
      if (error.message && error.message.includes('question_id')) {
        toast.error('后端接口错误：需要 question_id 参数，但接口文档未说明。请联系后端同事修复。');
      } else {
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity 
            style={[
              styles.publishBtn, 
              (!supplementAnswerText.trim() || isSubmitting) && styles.publishBtnDisabled
            ]}
            onPress={handleSubmit}
            disabled={!supplementAnswerText.trim() || isSubmitting}
          >
            <Text style={[
              styles.publishText, 
              (!supplementAnswerText.trim() || isSubmitting) && styles.publishTextDisabled
            ]}>
              {isSubmitting ? '发布中...' : '发布'}
            </Text>
          </TouchableOpacity>
        ...
        <View style={styles.originalAnswerContext}>
          ...
          <View style={styles.originalAnswerAuthor}>
            <Avatar 
              ...
              name={answer.userName || answer.userNickname || answer.author || '匿名用户'} 
              size={24} 
            />
        ...
        <View style={styles.toolbar}>
          <View style={styles.toolsLeft}>
            <TouchableOpacity 
              ...
              disabled={uploadingImages || images.length >= 9}
            >
              <Ionicons 
                name="image-outline" 
                size={24} 
                color={uploadingImages || images.length >= 9 ? '#ccc' : '#666'} 
              />
              {images.length > 0 && (
